...
$old_error_handler = set_error_handler("form_cliente_scriptcase_error_handler");

function form_cliente_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_cliente']['contr_erro']) && $_SESSION['scriptcase']['form_cliente']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_cliente->contr_form_cliente->Apl_com_erro = true;
            if ($inicial_form_cliente->contr_form_cliente->NM_ajax_flag)
            {
                $inicial_form_cliente->contr_form_cliente->NM_ajax_info['errList']['geral_form_cliente'][] = $msg;
                return;
            }
            form_cliente_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_cliente_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_cliente'])  && '' != $_SESSION['scriptcase']['error_icon']['form_cliente'])  ? $_SESSION['scriptcase']['error_icon']['form_cliente']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_cliente']) && '' != $_SESSION['scriptcase']['error_close']['form_cliente']) ? $_SESSION['scriptcase']['error_close']['form_cliente'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_cliente']) && '' != $_SESSION['scriptcase']['error_span_title']['form_cliente']) ? $_SESSION['scriptcase']['error_span_title']['form_cliente'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_cliente']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_cliente']) ? $_SESSION['scriptcase']['error_icon_title']['form_cliente'] : '';
?>
