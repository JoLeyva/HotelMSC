...
$_SESSION['scriptcase']['app_form_add_users_mob']['contr_erro'] = 'off';
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['dados_form']))
      {
          ...
          if (!isset($this->actives)){$this->actives = $this->nmgp_dados_form['actives'];} 
      }
...
//
      ...
      if ($this->nmgp_opcao == "recarga" || $this->nmgp_opcao == "recarga_mobile" || $this->nmgp_opcao == "muda_form") 
      {
          ...
          if (is_array($this->actives))
          {
              $x = 0; 
              $this->actives_1 = $this->actives;
              $this->actives = ""; 
              if ($this->actives_1 != "") 
              { 
                  foreach ($this->actives_1 as $dados_actives_1 ) 
                  { 
                      if ($x != 0)
                      { 
                          $this->actives .= ";";
                      } 
                      $this->actives .= $dados_actives_1;
                      $x++ ; 
                  } 
              } 
          } 
      ...
      if ($this->groups != "")
      {
          if (is_array($this->groups))
          {
              ...
              foreach ($this->groups_1 as $ind => $dados_groups_1 ) 
              {
                  ...
                  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_groups']) && !in_array($dados_groups_1, $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_groups']))
                  {
                      $hasError = true;
...
//--------------------------------------------------------------------------------------
   ...
   function nm_guardar_campos()
   {
    ...
    $this->nmgp_dados_form['actives'] = $this->actives;
...
$aLookup[] = array(app_form_add_users_mob_pack_protect_string('Y') => str_replace('<', '&lt;',app_form_add_users_mob_pack_protect_string("")));
$_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_active'][] = 'Y';
          $aLookupOrig = $aLookup;
          $sOptComp = "";
          ...
          {
              $this->NM_ajax_info['fldList']['active']['valList'][$i] = app_form_add_users_mob_pack_protect_string($v);
          }
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_groups']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_groups'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_groups']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_groups'] = array(); 
}
   ...
   if (!empty($this->actives))
   {
       if (is_array($this->actives))
       {
           $Tmp_array = $this->actives;
       }
       else
       {
           $Tmp_array = explode(";", $this->actives);
       }
       $actives_val_str = "";
       foreach ($Tmp_array as $Tmp_val_cmp)
       {
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $aLookup[] = array(app_form_add_users_mob_pack_protect_string(NM_charset_to_utf8($rs->fields[0])) => str_replace('<', '&lt;', app_form_add_users_mob_pack_protect_string(NM_charset_to_utf8($rs->fields[1]))));
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['app_form_add_users_mob']['Lookup_groups'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php 
   ...
   ob_end_clean();
          ...
          {
              $this->NM_ajax_info['fldList']['groups']['valList'][$i] = app_form_add_users_mob_pack_protect_string($v);
          }
          foreach ($aLookupOrig as $aValData)
          {
...
$_SESSION['scriptcase']['app_form_add_users_mob']['contr_erro'] = 'off'; 
      ...
      $NM_val_form['actives'] = $this->actives;
...
sc_send_mail_api(array(
    'profile' => '',
		...
		'settings' => [
				'gateway'       => 'smtp',
				'smtp_server'   => 'joel_sc@yopmail.com',
				'smtp_port'     => 'joel_sc@yopmail.com',
				'smtp_user'     => 'joel',
				'smtp_password' => '3001',
				'from_email'    => 'joel_sc@yopmail.com',
				'from_name'    => 'joel',
		],
    'message' => [
